...
struct GenerateStubMacroTests {
  ...
  func `Uses optional with fatalError for unknown custom return types`() {
    assertMacroExpansionSwiftTesting(
      ...
      diagnostics: [
        DiagnosticSpec(
          message: #"@GenerateStub: Custom types without known defaults use implicitly unwrapped optionals for properties and fatalError for methods. Use @DefaultValue for properties or @DefaultReturn for method returns."#,
          line: 1,
          column: 1,
          severity: .note,
        )
  ...
  func `Error on protocol with associated types`() {
    assertMacroExpansionSwiftTesting(
      ...
      diagnostics: [
        DiagnosticSpec(
          message: "@GenerateStub does not support protocols with associated types",
          line: 1,
          column: 1,
          severity: .error,
        )
  ...
  func `Error on protocol with static members`() {
    assertMacroExpansionSwiftTesting(
      ...
      diagnostics: [
        DiagnosticSpec(
          message: "@GenerateStub does not support static or class requirements",
          line: 1,
          column: 1,
          severity: .error,
        )
