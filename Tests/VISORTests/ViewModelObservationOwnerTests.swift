...
extension ViewModelObservationOwnerTests {
  @Test(
    "Generated LazyViewModel waits for readiness and joins on removal",
    .timeLimit(.minutes(1)))
  @MainActor
  func generatedLazyViewModelReadinessAndTeardown() async {
  ...
  @Test(
    "A mounted pause-when-inactive host follows the injected scene phase",
    .timeLimit(.minutes(1)))
  @MainActor
  func mountedPauseWhenInactiveScenePhaseLifetime() async {
    let service = OwnerService()
    let statusService = OwnerStatusService()
    let reactionGate = OwnerReactionGate()
    let viewModel = OwnerSourceBackedViewModel(
